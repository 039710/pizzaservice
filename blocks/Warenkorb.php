...
class Warenkorb        // to do: change name of class
{
    // --- ATTRIBUTES ---
    ...
    public function generateView($id = "", $url) 
    {
		...
		echo <<<EOF
        <div class="cart">
            <h3 id = "waren"> Warenkorb </h3>
          <form action="{$url}" method="POST">
            <select class="form-control" name="orders[]" id="orders" multiple size="20"></select>


            <div class ="form-group">
                <button class="btn btn-secondary btn-sm" type="reset" id="delete-all">Alle Löschen</button>
                <button class="btn btn-secondary btn-sm" type="button" id="delete-selected">Auswahl Löschen</button>
            </div>

           
            <div class ="form-group">
                <p class="text-left" >Total : <span id="sum">0.00</span> €</p>
                <label> Bitte Ihre Name und Adresse einfügen</label>
                <input type="text" placeholder="Name, Adresse" name="address" required id="address">
                
            </div>


            <button class="btn btn-success btn-sm" type="submit">Bestellen</button>
          </form>
        </div>
